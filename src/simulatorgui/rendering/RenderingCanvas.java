...
public class RenderingCanvas extends JPanel implements MouseInputListener, MouseWheelListener {
	...
	public void zoom(Point focus, double zoomTimes) {
		if (camTransform.getScaleX() <= MIN_ZOOM && zoomTimes < 1) {
			return;
		}
		if (camTransform.getScaleX() >= MAX_ZOOM && zoomTimes > 1) {
			return;
		}

//		zoomTimes = NumericUtilities.clamp(zoomTimes, MIN_ZOOM, MAX_ZOOM);
		...
		camTransform.scale(zoomTimes, zoomTimes);
		camTransform.translate(relX, relY);
	...
	public void mouseClicked(MouseEvent e) {
		...
		case MAKE_WIRE: {
			if (t instanceof NodeUI) {
				...
				((WireUI) currSelected).nodes.lastElement().setVisible(true);
				((WireUI) currSelected).addNode(temp);
			}
			break;
		}
		...
		case NONE: {
			if (t == null) {
				...
				if (t instanceof NodeUI) {
					...
					currSelected = new WireUI(this);
					Driver.getDriver().addWire((WireUI) currSelected);
					((WireUI) currSelected).addNode((NodeUI) t);
					var temp = new NodeUI(screenToLocalPoint(e.getLocationOnScreen()), this);
					temp.setVisible(false);
					((WireUI) currSelected).addNode(temp);
				} else if (t instanceof DeviceUI) {
					t.dispatchEvent(e);
				} else if (t instanceof WireUI) {
					t.dispatchEvent(e);
				}
			}
			break;
	...
	public void mouseMoved(MouseEvent e) {
		if (mode == currentMode.MAKE_WIRE) {
			((WireUI) currSelected).nodes.lastElement().setLocation(screenToLocalPoint(e.getLocationOnScreen()));
			Render();
		}
	}
}
